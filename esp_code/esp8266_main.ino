...
void setupWebServer() {
	...
	// Endpoint untuk unlock dengan PIN dan deny access
	server.on("/unlock-pin", HTTP_GET, handleUnlockPIN);
	server.on("/deny-access", HTTP_GET, handleDenyAccess);
...
void handleUnlockPIN() {
	server.sendHeader("Access-Control-Allow-Origin", "*");
	Serial.println("[PIN] PIN unlock triggered");
	unlockDoorWithPIN();
	server.send(200, "application/json", "{\"status\":\"unlocked\",\"method\":\"pin\"}");
}

// ===== Handle Deny Access (Wrong PIN or Face) =====
void handleDenyAccess() {
	server.sendHeader("Access-Control-Allow-Origin", "*");
	Serial.println("[DENY] Access denied - triggering alert");
	...
	// Trigger buzzer alert (3 long beeps)
	for (int i = 0; i < 3; i++) {
	...
	server.send(200, "application/json", "{\"status\":\"denied\"}");
}
...
void unlockDoorWithFace() {
	doorLocked = false;
	
	Serial.println("[ACTION] Unlocking door with face...");
	
	// Welcome beep (single short beep)
	digitalWrite(buzzPin, HIGH);
	delay(100);
	digitalWrite(buzzPin, LOW);
	
	// Tampilkan di LCD - Accuracy dan Welcome
	lcd.clear();
	lcd.setCursor(0, 0);
	lcd.print("Acc: " + String(lastConfidence, 1) + "%");
	lcd.setCursor(0, 1);
	lcd.print("Welcome!");
	...
	lcd.clear();
	lcd.setCursor(0, 0);
	if (lastRecognizedUser.length() > 16) {
		lcd.print(lastRecognizedUser.substring(0, 16));
	} else {
...
void unlockDoorWithPIN() {
	doorLocked = false;
	
	Serial.println("[ACTION] Unlocking door with PIN...");
	
	// Welcome beep (double short beep for PIN)
	digitalWrite(buzzPin, HIGH);
	delay(100);
	digitalWrite(buzzPin, LOW);
	delay(100);
	...
	Serial.println("[SUCCESS] Door unlocked with PIN!");
...
void lockDoor() {
	...
	// Update LCD to show locked status
	lcd.clear();
	lcd.setCursor(0, 0);
	lcd.print("Door: LOCKED");
	lcd.setCursor(0, 1);
...
void triggerAlert() {
	...
	// LED merah berkedip dengan buzzer (longer beeps for denied)
	for (int i = 0; i < 3; i++) {
		digitalWrite(buzzPin, HIGH);
		digitalWrite(ledRedPin, HIGH);
		delay(300); 	// Longer beep for denied
		digitalWrite(buzzPin, LOW);
		digitalWrite(ledRedPin, LOW);
		delay(300);
	}
